...
#'
#' Phase 0 ships `mm_formula_error`, `mm_schema_error`, and the
#' `mm_bridge_error` fallback for untagged Rust errors. Phase 1.A adds
#' `mm_data_error` (raised by `compile_model()` on data shape / type
#' problems). Phase 1.E adds `mm_fit_error` for fit construction /
#' optimization failures and `mm_inference_unavailable` for extractor or
#' prediction requests whose uncertainty contract is not implemented yet.
#' Subsequent phases add `mm_not_identifiable`, `mm_design_refusal`, and
