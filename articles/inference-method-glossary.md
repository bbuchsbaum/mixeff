...
| `interior_converged` *(route map)* | The fit is interior and the fast Wald route has no boundary caveat from the route map. An interior optimum says nothing about model specification, so the value deliberately does not claim it (the pre-0.2.0 spelling `interior_converged_well_specified` overclaimed and is retired). | Defined for the route map; superseded by the observed reason in current output. |
...
## `reliability` — the grade rubric

`reliability` is a closed four-value scale: `"high"`, `"moderate"`,
`"low"`, or `"not_available"`. This section is the rubric (rubric
version 1, mixeff 0.2.0): the exact mapping the code implements from
observable facts to a grade. The grade is a summary of *which method
produced the number and what is known about that method’s approximation*
— nothing more.

| Grade | Earned by | Where you see it |
|----|----|----|
| `high` | A model-based covariance route with no approximation beyond the model itself (the LMM `model_based` fixed-effect covariance). | LMM Wald/Satterthwaite covariance status. |
| `moderate` | A named, documented approximation within the validated envelope: finite-difference Satterthwaite df; the joint-Laplace active-Hessian GLMM covariance; the working-Hessian approximation under its explicit opt-in; a certified bootstrap with at least 999 successful replicates and a finite Monte Carlo SE. | Most certified inference rows. |
| `low` | The route ran but with a known weakening: a t target that fell back to standard-normal z (degenerate df), or a bootstrap below the 999-replicate floor. | Boundary-fit Wald rows; small-`nsim` bootstrap results. |
| `not_available` | No graded quantity: the row is a refusal, or reliability is not defined for the target. | Withheld GLMM rows; refusal rows. |

The 999-replicate bootstrap floor is a grading convention, documented
rather than asserted: at `B = 999` bootstrap p-values resolve on an
exact 1/(B+1) = 0.001 grid, and the Monte Carlo SE of a rate near 0.05
is about 0.0069. `B = 998` does not differ materially from `B = 999` —
which is why the replicate accounting (successful count, Monte Carlo SE)
ships alongside the grade, so you can judge the evidence directly
instead of trusting the label.

What a grade does **not** mean: no grade asserts that the model is
correctly specified, that the estimate is unbiased, or that a p-value is
exactly calibrated. Those are properties of the model and the data;
mixeff’s measured calibration evidence for its routes lives in the
benchmarking article’s simulation study, with Wilson intervals and
replicate accounting.

## What “certified” means

Several surfaces use the word *certified*. It has one definition:

> The engine’s inference contract marks this estimator/target
> combination available: the required numerical invariants passed, the
> value was produced by the named method, and that method is inside the
> package’s validated support envelope.

“Certified” does **not** mean statistically guaranteed correct, and it
does not mean the model is well specified. An absent certificate is
never treated as a certificate: surfaces that cannot see a certified
inference table withhold rather than assume. The uncertified
alternatives are always explicitly labelled (for example
`wald_z_working_hessian` rows under the `inference = "working_hessian"`
opt-in).
