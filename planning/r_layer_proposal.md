...
- use lme4-compatible random-effect factor semantics: `(1 + factor | group)`
  uses the supplied contrast basis for the factor slope, while
  `(0 + factor | group)` uses cell-means/level-indicator coding even when a
  contrast basis is available. This mirrors `model.matrix(~ factor)` versus
  `model.matrix(~ 0 + factor)`. The artifact/audit should record the basis
  choice explicitly, for example `basis_kind = "supplied"` or
  `basis_kind = "cell_means"`, and explain when a no-intercept factor term uses
  cell-means coding despite an available contrast basis.
...
5. **Factor/contrast tests.** Verify R factor levels and contrast matrices
   ...
   deterministically. Include the lme4 semantic distinction that
   `(1 + factor | group)` uses the supplied contrast basis, while
   `(0 + factor | group)` uses cell-means coding and records that basis choice
   in the artifact/audit payload.
