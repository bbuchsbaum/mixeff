...
# Vendored sources are present when EITHER the tarball form (rextendr's
# default at src/rust/vendor.tar.xz) OR the directory form (mixeff's
# tools/vendor-rust.R writes to src/vendor/) exists. Both are honored by
# src/Makevars.in. Detecting either is what gates `--offline` so cargo
# never reaches the network during R CMD check.
vendor_exists <- file.exists("src/rust/vendor.tar.xz") ||
                 dir.exists("src/vendor")
