...
`expected_reliability_reason` is the reason `inference_options()` attaches
to a route on its map. `reliability_reason` is the reason the engine
attaches to an actual inference row when a verb runs. The vocabularies
overlap but are not identical, and where the engine has recorded an
observed reason for a route, `inference_options()` reports the observed
value in preference to its own prediction — so on a fitted LMM the Wald
route usually shows the engine's `asymptotic_wald_z_fallback` rather than
the two route-map entries marked *(route map)* below.
...
| `asymptotic_wald_z_fallback` | A t reference distribution was the target, degrees of freedom could not be computed, and a standard normal was substituted; the row is graded `low`. | `inference_table()` and `summary()` on a boundary fit report their Wald rows with this reason. This is the reason code most users meet first. |
| `asymptotic_wald_z_at_boundary` *(route map)* | Wald z can run, but boundary or reduced-rank fits can make asymptotic uncertainty too optimistic. | Defined for the `inference_options()` route map; in practice the engine's observed `asymptotic_wald_z_fallback` takes precedence. |
...
| `bootstrap_monte_carlo_replicates` *(route map)* | Reliability is governed by replicate count and Monte-Carlo standard error. | The `inference_options()` prediction for bootstrap routes; the observed row value is `parametric_bootstrap_monte_carlo`. |
...
withheld (see `vignette("glmm")`).

## `reliability` — the grade rubric

`reliability` is a closed four-value scale: `"high"`, `"moderate"`,
`"low"`, or `"not_available"`. This section is the rubric (rubric version
1, mixeff 0.2.0): the exact mapping the code implements from observable
facts to a grade. The grade is a summary of *which method produced the
number and what is known about that method's approximation* — nothing
more.
...
rather than asserted: at `B = 999` bootstrap p-values resolve on an exact
1/(B+1) = 0.001 grid, and the Monte Carlo SE of a rate near 0.05 is about
0.0069. `B = 998` does not differ materially from `B = 999` — which is
why the replicate accounting (successful count, Monte Carlo SE) ships
alongside the grade, so you can judge the evidence directly instead of
trusting the label.
...
> The engine's inference contract marks this estimator/target combination
> available: the required numerical invariants passed, the value was
> produced by the named method, and that method is inside the package's
> validated support envelope.

"Certified" does **not** mean statistically guaranteed correct, and it
does not mean the model is well specified. An absent certificate is never
treated as a certificate: surfaces that cannot see a certified inference
table withhold rather than assume. The uncertified alternatives are
always explicitly labelled (for example `wald_z_working_hessian` rows
under the `inference = "working_hessian"` opt-in).
