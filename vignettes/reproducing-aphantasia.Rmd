...
control group. The intact and combined high-baseline Bernoulli models keep the
runnable profiled path in the opt-in reproduction gate and are checked against
the parity ledger. For the intact model, the labelled joint audit route
(`method = "joint_laplace"`, `mm_control(max_feval = 40)`) substantially
reduces the lme4 gap, but still returns an explicit budget-limited,
non-certified optimizer diagnostic. The combined model remains on the profiled
ledger path at this pin.
